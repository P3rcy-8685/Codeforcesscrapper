...
async function chart(){
    ...
    async function blog(){
        ...
        async function friend(){
            ...
            const secret=//enter secret key from codeforces 
            const key = //enter public key from codeforces 
            ...
            if (data.status=="OK"){
                var p="Ye le Dost";
                document.getElementById("Data").innerHTML=p;
                document.getElementById("Contributions").innerHTML="Online Friends are: "
                for (let i=0;i<Object.keys(data.result).length;i++){
                    document.getElementById("Contributions").innerHTML+=data.result[i]
                }}
            else{
                document.getElementById("Data").innerHTML="Bhai handle toh check karliya kar"}
            url="https://codeforces.com/api/user.friends?onlyOnline=false&apiKey="+key+"&time="+ Math.round((new Date()).getTime() / 1000)+"&apiSig=123456"+SHA512("123456/user.friends?apiKey="+key+"&onlyOnline=false"+"&time="+ Math.round((new Date()).getTime() / 1000)+"#"+sec)
            var resp=await fetch(url);
            var data= await resp.json();
            console.log(url)
            console.log(data);
            if (data.status=="OK"){
                document.getElementById("time").innerHTML="All Friends are: "
                for (let i=0;i<Object.keys(data.result).length;i++){
                    document.getElementById("time").innerHTML+=data.result[i]+", "
                }
            }
            else{
                document.getElementById("Data").innerHTML="Bhai handle toh check karliya kar"}
